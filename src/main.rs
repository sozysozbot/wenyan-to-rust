...
#[derive(Eq, PartialEq, Debug, Clone)]
struct IntNum(Vec<IntNumKeywords>);

#[derive(Eq, PartialEq, Debug, Clone)]
enum FloatNumKeywords {
    /// 分
    Fen1,
    /// 釐
    Li2,
    ...
    loop {
        ...
        match c {
            ...
            '「' => {
                let peek = iter.peek();
                match peek {
                    None => return Err(ParseError::NonterminatedIdentifier),
                    Some('「') => {
                        ...
                        loop {
                            let next = iter.next();
                            match next {
                                None => return Err(ParseError::NonterminatedStringLiteral),
                                Some('」') => match iter.next() {
                                    None => return Err(ParseError::NonterminatedStringLiteral),
                                    Some('」') => break,
                                    Some(a) => {
                                        return Err(ParseError::UnexpectedCharAfter('」', a))
                                    }
                                },
            ...
            '吾' => {
                let next = iter.next();
                match next {
                    None => return Err(ParseError::UnexpectedEOFAfter('吾')),
                    Some('有') => ans.push(Lex::Wu2You3),
                    Some('嘗') => match iter.next() {
                        Some('觀') => ans.push(Lex::Wu2Chang2Guan1),
                        None => return Err(ParseError::UnexpectedEOFAfter('嘗')),
                        Some(a) => return Err(ParseError::UnexpectedCharAfter('嘗', a)),
                    },
                    Some(a) => return Err(ParseError::UnexpectedCharAfter('吾', a)),
                }
            }
            '書' => {
                let next = iter.next();
                match next {
                    None => return Err(ParseError::UnexpectedEOFAfter('書')),
                    Some('之') => ans.push(Lex::Shu1Zhi1),
                    Some(a) => return Err(ParseError::UnexpectedCharAfter('書', a)),
                }
            }
            '今' => {
                let peek = iter.peek();
